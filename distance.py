...
def compute_lambda_distance(dm,eta,psi,t=100,P=None,alpha=None):
    # Compute Lambda-distance
    if eta == -1:
        _,dm_lambda = lambda_feature_infty_dm(dm,psi,t,P,alpha)
    elif eta > 0:
        _,dm_lambda = lambda_feature_continous_dm(dm,eta,psi,t,P,alpha)
    return dm_lambda
...
def get_pd_dm(dm):
    ...
    # diag_2 = np.array([(1,list(x[1])) for x in diag if x[0]==2])
